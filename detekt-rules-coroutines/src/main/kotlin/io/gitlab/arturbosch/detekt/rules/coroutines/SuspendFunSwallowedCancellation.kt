...
class SuspendFunSwallowedCancellation(config: Config) : Rule(config, "`runCatching` does not propagate `CancellationException`, don't use it with `suspend` lambda blocks.") {
